...
const VerticalFilters = ({ filters, handleChange, handleClear }) => {
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-2 gap-5 lg:gap-16 px-16 lg:px-28 py-10 border-b-light-green border-b-2">
      {filters.map((filter) => (
        <FilterInput
          id={filter.name}
          key={filter.name}
